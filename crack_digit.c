...
// as COMP1511 sample solution ande edited as COMP1511 Assignment
...
// This function analyzes the image in order to determine the digit. It begins by creating a bounding box around the digit and then calling various attributes to determine the digit. It will the ouput the digit into stdout.
void analyze_image(int height, int width, int pixels[height][width]) {
    int start_row, start_column, box_width, box_height, which_hole;
    ...
    //Finds the density of the black pixels for the digit
    double density = get_density(box_height, box_width, box_pixels);

    //Finds the density of black pixels for the lower proprtion of the digit 
    double lower_density = get_lower_density(box_height, box_width, box_pixels);

    //Finds the vertical balance of the digit
    double vertical_balance = get_vertical_balance(box_height, box_width, box_pixels);
    
    //Finds the number of holes in the digit, the hole_fraction and the position of the holes (if any)
    int hole = get_holes(box_height, box_width, box_pixels, &hole_fraction, &which_hole);

    //Finds the height/width of the bounding box
    double tallness = get_tallness(box_height, box_width, box_pixels);

    //Determines the densty of black pixels to the right hand side of the bouding box
